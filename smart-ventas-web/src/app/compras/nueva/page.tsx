...
export default function NuevaCompraPage() {
  ...
  const actualizarCantidad = (idx: number, cantidad: number) => {
    ...
    updated[idx] = {
      ...
      subtotal: cantidad * updated[idx].costoUnitario,
    };
    setItems(updated);
  };

  const actualizarCosto = (idx: number, costo: number) => {
    const updated = [...items];
    updated[idx] = {
      ...updated[idx],
      costoUnitario: Math.max(0, costo),
      subtotal: updated[idx].cantidad * Math.max(0, costo),
    };
    setItems(updated);
  };
  ...
  const guardar = async () => {
    if (items.length === 0 || !proveedorNombre.trim()) return;
    setGuardando(true);
    try {
    ...
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-5 max-w-3xl mx-auto">
      <motion.div variants={itemVariants} className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-[var(--text-secondary)] mt-1">Registra una nueva compra a proveedor</p>
        </div>
        <button onClick={() => router.push('/compras')} className="btn-secondary">
          <X size={16} /> Cancelar
        </button>
      </motion.div>
            ...
            {items.map((item, idx) => (
              <div key={idx} className="flex items-center gap-3 p-3 rounded-xl bg-[var(--bg-tertiary)]">
                ...
                <input
                  ...
                  onChange={e => actualizarCosto(idx, parseFloat(e.target.value) || 0)}
      ...
      <motion.div variants={itemVariants} className="flex justify-end">
        <button
          onClick={guardar}
          disabled={items.length === 0 || !proveedorNombre.trim() || guardando}
